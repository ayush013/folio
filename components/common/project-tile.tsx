...
const ProjectTile = ({ project }) => {
    ...
    return (
        <a href={project.url} target='_blank' rel='noreferrer' className='link max-w-full overflow-hidden rounded-3xl'>
            <div ref={projectCard} className={styles.ProjectTile + ' rounded-3xl relative p-6 flex-col flex justify-between max-w-full'} style={{ background: `linear-gradient(90deg, ${project.gradient[0]} 0%, ${project.gradient[1]} 100%)` }}>
                ...
                <h1 className='text-3xl sm:text-4xl z-10 pl-2' style={{ transform: 'translateZ(3rem)' }}>{project.name}</h1>
                ...
                <h2 className='text-xl z-10 tracking-wide font-medium' style={{ transform: 'translateZ(0.8rem)' }}>{project.description}</h2>
            </div>
        </a>
    )
}
