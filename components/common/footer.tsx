            ...
            <div className='h-full w-full'>
                <div className='2xl:container mx-auto xl:px-20 md:px-12 px-4 flex-col flex h-full justify-end z-10 items-center py-12'>
                    ...
                    <div className='flex mt-8'>
                        {Object.keys(SOCIAL_LINKS).map(el => <a href={SOCIAL_LINKS[el]} key={el} className='link hover:opacity-80 duration-300 md:px-2 px-1' rel='noreferrer' target='_blank'>
                            <Image src={`/social/${el}.svg`} alt={el} width={40} height={40} />
                        </a>
                        )}
                    </div>
                    <div className='flex mt-8'>
                        <Button classes='mr-3' type='outline' name='Resume' newTab={true} href='/Ayush_Resume.pdf'></Button>
                        <Button classes='ml-3' type='white' name={`Let's Talk`} href={'mailto:' + EMAIL}></Button>
                    </div>
                    <p className='text-center text-sm sm:text-base mt-8'>Designed and Developed with ❤️ by Ayush</p>
                </div>
            </div>
            <style jsx global>{`
            footer {
